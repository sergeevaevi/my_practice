...
void main_func(Ui::MainWindow * ui) {
    ...
    printf("\n%u and %u\n", C_final[4].Win[2].is_available, C_final[3].Win[11].t_AOS);
    ...
    char szbuff[200] = "";
    printContact(C_final);
    printf("\n%u and %u\n", C_final[1].Win[12].is_final,  C_final[1].Win[13].is_final);
     printContact(C);
    for (i = 0; i < NUM_REQS; i++) {
        for (j = 0; j < C[i].num_wins; j++) {
    ...
    for (k = 0; k < NUM_REQS; k++) {
        ...
        label->setObjectName("label_"+QString::number(count));
        ui->gridLayout->addWidget(label, j, k, 1, 1);
...
//        for(int j = 0; j < 4;j++){
//            QLabel * label = new QLabel(ui->gridLayoutWidget);
//            label->setText("Me is "+QString::number(count));
//            label->setObjectName("label_"+QString::number(count));
